...
CIMGUI_API ImGuiContext* ImGuiCreateContext(ImFontAtlas* shared_font_atlas /* = NULL */);
CIMGUI_API void          ImGuiDestroyContext(ImGuiContext* ctx /* = NULL */);   // NULL = destroy current context
CIMGUI_API ImGuiContext* ImGuiGetCurrentContext(void);
CIMGUI_API void          ImGuiSetCurrentContext(ImGuiContext* ctx);

// Main
CIMGUI_API ImGuiIO*         ImGuiGetIO(void);           // access the ImGuiIO structure (mouse/keyboard/gamepad inputs, time, various configuration options/flags)
CIMGUI_API ImGuiPlatformIO* ImGuiGetPlatformIO(void);   // access the ImGuiPlatformIO structure (mostly hooks/functions to connect to platform/renderer and OS Clipboard, IME etc.)
CIMGUI_API ImGuiStyle*      ImGuiGetStyle(void);        // access the Style structure (colors, sizes). Always use PushStyleColor(), PushStyleVar() to modify style mid-frame!
CIMGUI_API void             ImGuiNewFrame(void);        // start a new Dear ImGui frame, you can submit any command from this point until Render()/EndFrame().
CIMGUI_API void             ImGuiEndFrame(void);        // ends the Dear ImGui frame. automatically called by Render(). If you don't need to render data (skipping rendering) you may call EndFrame() without Render()... but you'll have wasted CPU already! If you don't need to render, better to not create any windows and not call NewFrame() at all!
CIMGUI_API void             ImGuiRender(void);          // ends the Dear ImGui frame, finalize the draw data. You can then get call GetDrawData().
CIMGUI_API ImDrawData*      ImGuiGetDrawData(void);     // valid after Render() and until the next call to NewFrame(). Call ImGui_ImplXXXX_RenderDrawData() function in your Renderer Backend to render.

// Demo, Debug, Information
CIMGUI_API void        ImGuiShowDemoWindow(bool* p_open /* = NULL */);            // create Demo window. demonstrate most ImGui features. call this to learn about the library! try to make it always available in your application!
CIMGUI_API void        ImGuiShowMetricsWindow(bool* p_open /* = NULL */);         // create Metrics/Debugger window. display Dear ImGui internals: windows, draw commands, various internal state, etc.
CIMGUI_API void        ImGuiShowDebugLogWindow(bool* p_open /* = NULL */);        // create Debug Log window. display a simplified log of important dear imgui events.
CIMGUI_API void        ImGuiShowIDStackToolWindow(void);                          // Implied p_open = NULL
CIMGUI_API void        ImGuiShowIDStackToolWindowEx(bool* p_open /* = NULL */);   // create Stack Tool window. hover items with mouse to query information about the source of their unique ID.
CIMGUI_API void        ImGuiShowAboutWindow(bool* p_open /* = NULL */);           // create About window. display Dear ImGui version, credits and build/system information.
CIMGUI_API void        ImGuiShowStyleEditor(ImGuiStyle* ref /* = NULL */);        // add style editor block (not a window). you can pass in a reference ImGuiStyle structure to compare to, revert to and save to (else it uses the default style)
CIMGUI_API bool        ImGuiShowStyleSelector(const char* label);                 // add style selector block (not a window), essentially a combo listing the default styles.
CIMGUI_API void        ImGuiShowFontSelector(const char* label);                  // add font selector block (not a window), essentially a combo listing the loaded fonts.
CIMGUI_API void        ImGuiShowUserGuide(void);                                  // add basic help/info block (not a window): how to manipulate ImGui as an end-user (mouse/keyboard controls).
CIMGUI_API const char* ImGuiGetVersion(void);                                     // get the compiled version string e.g. "1.80 WIP" (essentially the value for IMGUI_VERSION from the compiled version of imgui.cpp)

// Styles
CIMGUI_API void ImGuiStyleColorsDark(ImGuiStyle* dst /* = NULL */);      // new, recommended style (default)
CIMGUI_API void ImGuiStyleColorsLight(ImGuiStyle* dst /* = NULL */);     // best used with borders and a custom, thicker font
CIMGUI_API void ImGuiStyleColorsClassic(ImGuiStyle* dst /* = NULL */);   // classic imgui style
...
CIMGUI_API bool ImGuiBegin(const char* name, bool* p_open /* = NULL */, ImGuiWindowFlags flags /* = 0 */);
CIMGUI_API void ImGuiEnd(void);
...
//    such as BeginMenu/EndMenu, BeginPopup/EndPopup, etc. where the EndXXX call should only be called if the corresponding
//    BeginXXX function returned true. Begin and BeginChild are the only odd ones out. Will be fixed in a future update.]
CIMGUI_API bool ImGuiBeginChild(const char* str_id, ImVec2 size /* = ImVec2(0, 0) */, ImGuiChildFlags child_flags /* = 0 */, ImGuiWindowFlags window_flags /* = 0 */);
CIMGUI_API bool ImGuiBeginChildID(ImGuiID id, ImVec2 size /* = ImVec2(0, 0) */, ImGuiChildFlags child_flags /* = 0 */, ImGuiWindowFlags window_flags /* = 0 */);
CIMGUI_API void ImGuiEndChild(void);

// Windows Utilities
// - 'current window' = the window we are appending into while inside a Begin()/End() block. 'next window' = next window we will Begin() into.
CIMGUI_API bool        ImGuiIsWindowAppearing(void);
CIMGUI_API bool        ImGuiIsWindowCollapsed(void);
CIMGUI_API bool        ImGuiIsWindowFocused(ImGuiFocusedFlags flags /* = 0 */);   // is current window focused? or its root/child, depending on flags. see flags for options.
CIMGUI_API bool        ImGuiIsWindowHovered(ImGuiHoveredFlags flags /* = 0 */);   // is current window hovered and hoverable (e.g. not blocked by a popup/modal)? See ImGuiHoveredFlags_ for options. IMPORTANT: If you are trying to check whether your mouse should be dispatched to Dear ImGui or to your underlying app, you should not use this function! Use the 'io.WantCaptureMouse' boolean for that! Refer to FAQ entry "How can I tell whether to dispatch mouse/keyboard to Dear ImGui or my application?" for details.
CIMGUI_API ImDrawList* ImGuiGetWindowDrawList(void);                              // get draw list associated to the current window, to append your own drawing primitives
CIMGUI_API ImVec2      ImGuiGetWindowPos(void);                                   // get current window position in screen space (IT IS UNLIKELY YOU EVER NEED TO USE THIS. Consider always using GetCursorScreenPos() and GetContentRegionAvail() instead)
CIMGUI_API ImVec2      ImGuiGetWindowSize(void);                                  // get current window size (IT IS UNLIKELY YOU EVER NEED TO USE THIS. Consider always using GetCursorScreenPos() and GetContentRegionAvail() instead)
CIMGUI_API float       ImGuiGetWindowWidth(void);                                 // get current window width (IT IS UNLIKELY YOU EVER NEED TO USE THIS). Shortcut for GetWindowSize().x.
CIMGUI_API float       ImGuiGetWindowHeight(void);                                // get current window height (IT IS UNLIKELY YOU EVER NEED TO USE THIS). Shortcut for GetWindowSize().y.

// Window manipulation
// - Prefer using SetNextXXX functions (before Begin) rather that SetXXX functions (after Begin).
CIMGUI_API void ImGuiSetNextWindowPos(ImVec2 pos, ImGuiCond cond /* = 0 */);                                        // Implied pivot = ImVec2(0, 0)
CIMGUI_API void ImGuiSetNextWindowPosEx(ImVec2 pos, ImGuiCond cond /* = 0 */, ImVec2 pivot /* = ImVec2(0, 0) */);   // set next window position. call before Begin(). use pivot=(0.5f,0.5f) to center on given point, etc.
CIMGUI_API void ImGuiSetNextWindowSize(ImVec2 size, ImGuiCond cond /* = 0 */);                                      // set next window size. set axis to 0.0f to force an auto-fit on this axis. call before Begin()
CIMGUI_API void ImGuiSetNextWindowSizeConstraints(ImVec2 size_min, ImVec2 size_max, ImGuiSizeCallback custom_callback /* = NULL */, void* custom_callback_data /* = NULL */); // set next window size limits. use 0.0f or FLT_MAX if you don't want limits. Use -1 for both min and max of same axis to preserve current size (which itself is a constraint). Use callback to apply non-trivial programmatic constraints.
CIMGUI_API void ImGuiSetNextWindowContentSize(ImVec2 size);                                                         // set next window content size (~ scrollable client area, which enforce the range of scrollbars). Not including window decorations (title bar, menu bar, etc.) nor WindowPadding. set an axis to 0.0f to leave it automatic. call before Begin()
CIMGUI_API void ImGuiSetNextWindowCollapsed(bool collapsed, ImGuiCond cond /* = 0 */);                              // set next window collapsed state. call before Begin()
CIMGUI_API void ImGuiSetNextWindowFocus(void);                                                                      // set next window to be focused / top-most. call before Begin()
CIMGUI_API void ImGuiSetNextWindowScroll(ImVec2 scroll);                                                            // set next window scrolling value (use < 0.0f to not affect a given axis).
CIMGUI_API void ImGuiSetNextWindowBgAlpha(float alpha);                                                             // set next window background color alpha. helper to easily override the Alpha component of ImGuiCol_WindowBg/ChildBg/PopupBg. you may also use ImGuiWindowFlags_NoBackground.
CIMGUI_API void ImGuiSetWindowPos(ImVec2 pos, ImGuiCond cond /* = 0 */);                                            // (not recommended) set current window position - call within Begin()/End(). prefer using SetNextWindowPos(), as this may incur tearing and side-effects.
CIMGUI_API void ImGuiSetWindowSize(ImVec2 size, ImGuiCond cond /* = 0 */);                                          // (not recommended) set current window size - call within Begin()/End(). set to ImVec2(0, 0) to force an auto-fit. prefer using SetNextWindowSize(), as this may incur tearing and minor side-effects.
CIMGUI_API void ImGuiSetWindowCollapsed(bool collapsed, ImGuiCond cond /* = 0 */);                                  // (not recommended) set current window collapsed state. prefer using SetNextWindowCollapsed().
CIMGUI_API void ImGuiSetWindowFocus(void);                                                                          // (not recommended) set current window to be focused / top-most. prefer using SetNextWindowFocus().
CIMGUI_API void ImGuiSetWindowPosStr(const char* name, ImVec2 pos, ImGuiCond cond /* = 0 */);                       // set named window position.
CIMGUI_API void ImGuiSetWindowSizeStr(const char* name, ImVec2 size, ImGuiCond cond /* = 0 */);                     // set named window size. set axis to 0.0f to force an auto-fit on this axis.
CIMGUI_API void ImGuiSetWindowCollapsedStr(const char* name, bool collapsed, ImGuiCond cond /* = 0 */);             // set named window collapsed state
CIMGUI_API void ImGuiSetWindowFocusStr(const char* name);                                                           // set named window to be focused / top-most. use NULL to remove focus.

// Windows Scrolling
// - Any change of Scroll will be applied at the beginning of next frame in the first call to Begin().
// - You may instead use SetNextWindowScroll() prior to calling Begin() to avoid this delay, as an alternative to using SetScrollX()/SetScrollY().
CIMGUI_API float ImGuiGetScrollX(void);                                                      // get scrolling amount [0 .. GetScrollMaxX()]
CIMGUI_API float ImGuiGetScrollY(void);                                                      // get scrolling amount [0 .. GetScrollMaxY()]
CIMGUI_API void  ImGuiSetScrollX(float scroll_x);                                            // set scrolling amount [0 .. GetScrollMaxX()]
CIMGUI_API void  ImGuiSetScrollY(float scroll_y);                                            // set scrolling amount [0 .. GetScrollMaxY()]
CIMGUI_API float ImGuiGetScrollMaxX(void);                                                   // get maximum scrolling amount ~~ ContentSize.x - WindowSize.x - DecorationsSize.x
CIMGUI_API float ImGuiGetScrollMaxY(void);                                                   // get maximum scrolling amount ~~ ContentSize.y - WindowSize.y - DecorationsSize.y
CIMGUI_API void  ImGuiSetScrollHereX(float center_x_ratio /* = 0.5f */);                     // adjust scrolling amount to make current cursor position visible. center_x_ratio=0.0: left, 0.5: center, 1.0: right. When using to make a "default/current item" visible, consider using SetItemDefaultFocus() instead.
CIMGUI_API void  ImGuiSetScrollHereY(float center_y_ratio /* = 0.5f */);                     // adjust scrolling amount to make current cursor position visible. center_y_ratio=0.0: top, 0.5: center, 1.0: bottom. When using to make a "default/current item" visible, consider using SetItemDefaultFocus() instead.
CIMGUI_API void  ImGuiSetScrollFromPosX(float local_x, float center_x_ratio /* = 0.5f */);   // adjust scrolling amount to make given position visible. Generally GetCursorStartPos() + offset to compute a valid position.
CIMGUI_API void  ImGuiSetScrollFromPosY(float local_y, float center_y_ratio /* = 0.5f */);   // adjust scrolling amount to make given position visible. Generally GetCursorStartPos() + offset to compute a valid position.
...
CIMGUI_API void         ImGuiPushFontFloat(ImFont* font, float font_size_base_unscaled);   // Use NULL as a shortcut to keep current font. Use 0.0f to keep current size.
CIMGUI_API void         ImGuiPopFont(void);
CIMGUI_API ImFont*      ImGuiGetFont(void);                                                // get current font
CIMGUI_API float        ImGuiGetFontSize(void);                                            // get current scaled font size (= height in pixels). AFTER global scale factors applied. *IMPORTANT* DO NOT PASS THIS VALUE TO PushFont()! Use ImGui::GetStyle().FontSizeBase to get value before global scale factors.
CIMGUI_API ImFontBaked* ImGuiGetFontBaked(void);                                           // get current font bound at current size // == GetFont()->GetFontBaked(GetFontSize())

// Parameters stacks (shared)
CIMGUI_API void ImGuiPushStyleColor(ImGuiCol idx, ImU32 col);             // modify a style color. always use this if you modify the style after NewFrame().
CIMGUI_API void ImGuiPushStyleColorImVec4(ImGuiCol idx, ImVec4 col);
CIMGUI_API void ImGuiPopStyleColor(void);                                 // Implied count = 1
CIMGUI_API void ImGuiPopStyleColorEx(int count /* = 1 */);
CIMGUI_API void ImGuiPushStyleVar(ImGuiStyleVar idx, float val);          // modify a style float variable. always use this if you modify the style after NewFrame()!
CIMGUI_API void ImGuiPushStyleVarImVec2(ImGuiStyleVar idx, ImVec2 val);   // modify a style ImVec2 variable. "
CIMGUI_API void ImGuiPushStyleVarX(ImGuiStyleVar idx, float val_x);       // modify X component of a style ImVec2 variable. "
CIMGUI_API void ImGuiPushStyleVarY(ImGuiStyleVar idx, float val_y);       // modify Y component of a style ImVec2 variable. "
CIMGUI_API void ImGuiPopStyleVar(void);                                   // Implied count = 1
CIMGUI_API void ImGuiPopStyleVarEx(int count /* = 1 */);
CIMGUI_API void ImGuiPushItemFlag(ImGuiItemFlags option, bool enabled);   // modify specified shared item flag, e.g. PushItemFlag(ImGuiItemFlags_NoTabStop, true)
CIMGUI_API void ImGuiPopItemFlag(void);

// Parameters stacks (current window)
CIMGUI_API void  ImGuiPushItemWidth(float item_width);                        // push width of items for common large "item+label" widgets. >0.0f: width in pixels, <0.0f align xx pixels to the right of window (so -FLT_MIN always align width to the right side).
CIMGUI_API void  ImGuiPopItemWidth(void);
CIMGUI_API void  ImGuiSetNextItemWidth(float item_width);                     // set width of the _next_ common large "item+label" widget. >0.0f: width in pixels, <0.0f align xx pixels to the right of window (so -FLT_MIN always align width to the right side)
CIMGUI_API float ImGuiCalcItemWidth(void);                                    // width of item given pushed settings and current cursor position. NOT necessarily the width of last item unlike most 'Item' functions.
CIMGUI_API void  ImGuiPushTextWrapPos(float wrap_local_pos_x /* = 0.0f */);   // push word-wrapping position for Text*() commands. < 0.0f: no wrapping; 0.0f: wrap to end of window (or column); > 0.0f: wrap at 'wrap_pos_x' position in window local space
CIMGUI_API void  ImGuiPopTextWrapPos(void);

// Style read access
// - Use the ShowStyleEditor() function to interactively see/edit the colors.
CIMGUI_API ImVec2        ImGuiGetFontTexUvWhitePixel(void);                                  // get UV coordinate for a white pixel, useful to draw custom shapes via the ImDrawList API
CIMGUI_API ImU32         ImGuiGetColorU32(ImGuiCol idx);                                     // Implied alpha_mul = 1.0f
CIMGUI_API ImU32         ImGuiGetColorU32Ex(ImGuiCol idx, float alpha_mul /* = 1.0f */);     // retrieve given style color with style alpha applied and optional extra alpha multiplier, packed as a 32-bit value suitable for ImDrawList
CIMGUI_API ImU32         ImGuiGetColorU32ImVec4(ImVec4 col);                                 // retrieve given color with style alpha applied, packed as a 32-bit value suitable for ImDrawList
CIMGUI_API ImU32         ImGuiGetColorU32ImU32(ImU32 col);                                   // Implied alpha_mul = 1.0f
CIMGUI_API ImU32         ImGuiGetColorU32ImU32Ex(ImU32 col, float alpha_mul /* = 1.0f */);   // retrieve given color with style alpha applied, packed as a 32-bit value suitable for ImDrawList
CIMGUI_API const ImVec4* ImGuiGetStyleColorVec4(ImGuiCol idx);                               // retrieve style color as stored in ImGuiStyle structure. use to feed back into PushStyleColor(), otherwise use GetColorU32() to get style color with style alpha baked in.
...
CIMGUI_API ImVec2 ImGuiGetCursorScreenPos(void);         // cursor position, absolute coordinates. THIS IS YOUR BEST FRIEND (prefer using this rather than GetCursorPos(), also more useful to work with ImDrawList API).
CIMGUI_API void   ImGuiSetCursorScreenPos(ImVec2 pos);   // cursor position, absolute coordinates. THIS IS YOUR BEST FRIEND.
CIMGUI_API ImVec2 ImGuiGetContentRegionAvail(void);      // available space from current position. THIS IS YOUR BEST FRIEND.
CIMGUI_API ImVec2 ImGuiGetCursorPos(void);               // [window-local] cursor position in window-local coordinates. This is not your best friend.
CIMGUI_API float  ImGuiGetCursorPosX(void);              // [window-local] "
CIMGUI_API float  ImGuiGetCursorPosY(void);              // [window-local] "
CIMGUI_API void   ImGuiSetCursorPos(ImVec2 local_pos);   // [window-local] "
CIMGUI_API void   ImGuiSetCursorPosX(float local_x);     // [window-local] "
CIMGUI_API void   ImGuiSetCursorPosY(float local_y);     // [window-local] "
CIMGUI_API ImVec2 ImGuiGetCursorStartPos(void);          // [window-local] initial cursor position, in window-local coordinates. Call GetCursorScreenPos() after Begin() to get the absolute coordinates version.

// Other layout functions
CIMGUI_API void  ImGuiSeparator(void);                           // separator, generally horizontal. inside a menu bar or in horizontal layout mode, this becomes a vertical separator.
CIMGUI_API void  ImGuiSameLine(void);                            // Implied offset_from_start_x = 0.0f, spacing = -1.0f
CIMGUI_API void  ImGuiSameLineEx(float offset_from_start_x /* = 0.0f */, float spacing /* = -1.0f */); // call between widgets or groups to layout them horizontally. X position given in window coordinates.
CIMGUI_API void  ImGuiNewLine(void);                             // undo a SameLine() or force a new line when in a horizontal-layout context.
CIMGUI_API void  ImGuiSpacing(void);                             // add vertical spacing.
CIMGUI_API void  ImGuiDummy(ImVec2 size);                        // add a dummy item of given size. unlike InvisibleButton(), Dummy() won't take the mouse click or be navigable into.
CIMGUI_API void  ImGuiIndent(void);                              // Implied indent_w = 0.0f
CIMGUI_API void  ImGuiIndentEx(float indent_w /* = 0.0f */);     // move content position toward the right, by indent_w, or style.IndentSpacing if indent_w <= 0
CIMGUI_API void  ImGuiUnindent(void);                            // Implied indent_w = 0.0f
CIMGUI_API void  ImGuiUnindentEx(float indent_w /* = 0.0f */);   // move content position back to the left, by indent_w, or style.IndentSpacing if indent_w <= 0
CIMGUI_API void  ImGuiBeginGroup(void);                          // lock horizontal starting position
CIMGUI_API void  ImGuiEndGroup(void);                            // unlock horizontal starting position + capture the whole group bounding box into one "item" (so you can use IsItemHovered() or layout primitives such as SameLine() on whole group, etc.)
CIMGUI_API void  ImGuiAlignTextToFramePadding(void);             // vertically align upcoming text baseline to FramePadding.y so that it will align properly to regularly framed items (call if you have text on a line before a framed item)
CIMGUI_API float ImGuiGetTextLineHeight(void);                   // ~ FontSize
CIMGUI_API float ImGuiGetTextLineHeightWithSpacing(void);        // ~ FontSize + style.ItemSpacing.y (distance in pixels between 2 consecutive lines of text)
CIMGUI_API float ImGuiGetFrameHeight(void);                      // ~ FontSize + style.FramePadding.y * 2
CIMGUI_API float ImGuiGetFrameHeightWithSpacing(void);           // ~ FontSize + style.FramePadding.y * 2 + style.ItemSpacing.y (distance in pixels between 2 consecutive lines of framed widgets)
...
CIMGUI_API void    ImGuiPushID(const char* str_id);                                    // push string into the ID stack (will hash string).
CIMGUI_API void    ImGuiPushIDStr(const char* str_id_begin, const char* str_id_end);   // push string into the ID stack (will hash string).
CIMGUI_API void    ImGuiPushIDPtr(const void* ptr_id);                                 // push pointer into the ID stack (will hash pointer).
CIMGUI_API void    ImGuiPushIDInt(int int_id);                                         // push integer into the ID stack (will hash integer).
CIMGUI_API void    ImGuiPopID(void);                                                   // pop from the ID stack.
CIMGUI_API ImGuiID ImGuiGetID(const char* str_id);                                     // calculate unique ID (hash of whole ID stack + given parameter). e.g. if you want to query into ImGuiStorage yourself
CIMGUI_API ImGuiID ImGuiGetIDStr(const char* str_id_begin, const char* str_id_end);
CIMGUI_API ImGuiID ImGuiGetIDPtr(const void* ptr_id);
CIMGUI_API ImGuiID ImGuiGetIDInt(int int_id);

// Widgets: Text
CIMGUI_API void ImGuiTextUnformatted(const char* text);                                            // Implied text_end = NULL
CIMGUI_API void ImGuiTextUnformattedEx(const char* text, const char* text_end /* = NULL */);       // raw text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn't require null terminated string if 'text_end' is specified, B) it's faster, no memory copy is done, no buffer size limits, recommended for long chunks of text.
CIMGUI_API void ImGuiText(const char* fmt, ...) IM_FMTARGS(1);                                     // formatted text
CIMGUI_API void ImGuiTextV(const char* fmt, va_list args) IM_FMTLIST(1);
CIMGUI_API void ImGuiTextColored(ImVec4 col, const char* fmt, ...) IM_FMTARGS(2);                  // shortcut for PushStyleColor(ImGuiCol_Text, col); Text(fmt, ...); PopStyleColor();
CIMGUI_API void ImGuiTextColoredUnformatted(ImVec4 col, const char* text);                         // shortcut for PushStyleColor(ImGuiCol_Text, col); Text(fmt, ...); PopStyleColor();
CIMGUI_API void ImGuiTextColoredV(ImVec4 col, const char* fmt, va_list args) IM_FMTLIST(2);
CIMGUI_API void ImGuiTextDisabled(const char* fmt, ...) IM_FMTARGS(1);                             // shortcut for PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TextDisabled]); Text(fmt, ...); PopStyleColor();
CIMGUI_API void ImGuiTextDisabledUnformatted(const char* text);                                    // shortcut for PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TextDisabled]); Text(fmt, ...); PopStyleColor();
CIMGUI_API void ImGuiTextDisabledV(const char* fmt, va_list args) IM_FMTLIST(1);
CIMGUI_API void ImGuiTextWrapped(const char* fmt, ...) IM_FMTARGS(1);                              // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on an auto-resizing window if there's no other widgets to extend the window width, yoy may need to set a size using SetNextWindowSize().
CIMGUI_API void ImGuiTextWrappedUnformatted(const char* text);                                     // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on an auto-resizing window if there's no other widgets to extend the window width, yoy may need to set a size using SetNextWindowSize().
CIMGUI_API void ImGuiTextWrappedV(const char* fmt, va_list args) IM_FMTLIST(1);
CIMGUI_API void ImGuiLabelText(const char* label, const char* fmt, ...) IM_FMTARGS(2);             // display text+label aligned the same way as value+label widgets
CIMGUI_API void ImGuiLabelTextUnformatted(const char* label, const char* text);                    // display text+label aligned the same way as value+label widgets
CIMGUI_API void ImGuiLabelTextV(const char* label, const char* fmt, va_list args) IM_FMTLIST(2);
CIMGUI_API void ImGuiBulletText(const char* fmt, ...) IM_FMTARGS(1);                               // shortcut for Bullet()+Text()
CIMGUI_API void ImGuiBulletTextUnformatted(const char* text);                                      // shortcut for Bullet()+Text()
CIMGUI_API void ImGuiBulletTextV(const char* fmt, va_list args) IM_FMTLIST(1);
CIMGUI_API void ImGuiSeparatorText(const char* label);                                             // currently: formatted text with a horizontal line

// Widgets: Main
// - Most widgets return true when the value has been changed or when pressed/selected
// - You may also use one of the many IsItemXXX functions (e.g. IsItemActive, IsItemHovered, etc.) to query widget state.
CIMGUI_API bool ImGuiButton(const char* label);                                                                // Implied size = ImVec2(0, 0)
CIMGUI_API bool ImGuiButtonEx(const char* label, ImVec2 size /* = ImVec2(0, 0) */);                            // button
CIMGUI_API bool ImGuiSmallButton(const char* label);                                                           // button with (FramePadding.y == 0) to easily embed within text
CIMGUI_API bool ImGuiInvisibleButton(const char* str_id, ImVec2 size, ImGuiButtonFlags flags /* = 0 */);       // flexible button behavior without the visuals, frequently useful to build custom behaviors using the public api (along with IsItemActive, IsItemHovered, etc.)
CIMGUI_API bool ImGuiArrowButton(const char* str_id, ImGuiDir dir);                                            // square button with an arrow shape
CIMGUI_API bool ImGuiCheckbox(const char* label, bool* v);
CIMGUI_API bool ImGuiCheckboxFlagsIntPtr(const char* label, int* flags, int flags_value);
CIMGUI_API bool ImGuiCheckboxFlagsUintPtr(const char* label, unsigned int* flags, unsigned int flags_value);
CIMGUI_API bool ImGuiRadioButton(const char* label, bool active);                                              // use with e.g. if (RadioButton("one", my_value==1)) { my_value = 1; }
CIMGUI_API bool ImGuiRadioButtonIntPtr(const char* label, int* v, int v_button);                               // shortcut to handle the above pattern when value is an integer
CIMGUI_API void ImGuiProgressBar(float fraction, ImVec2 size_arg /* = ImVec2(-FLT_MIN, 0) */, const char* overlay /* = NULL */);
CIMGUI_API void ImGuiBullet(void);                                                                             // draw a small circle + keep the cursor on the same line. advance cursor x position by GetTreeNodeToLabelSpacing(), same distance that TreeNode() uses
CIMGUI_API bool ImGuiTextLink(const char* label);                                                              // hyperlink text button, return true when clicked
CIMGUI_API bool ImGuiTextLinkOpenURL(const char* label);                                                       // Implied url = NULL
CIMGUI_API bool ImGuiTextLinkOpenURLEx(const char* label, const char* url /* = NULL */);                       // hyperlink text button, automatically open file/url when clicked
...
CIMGUI_API void ImGuiImage(ImTextureRef tex_ref, ImVec2 image_size);                                                                       // Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1)
CIMGUI_API void ImGuiImageEx(ImTextureRef tex_ref, ImVec2 image_size, ImVec2 uv0 /* = ImVec2(0, 0) */, ImVec2 uv1 /* = ImVec2(1, 1) */);
CIMGUI_API void ImGuiImageWithBg(ImTextureRef tex_ref, ImVec2 image_size);                                                                 // Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1), bg_col = ImVec4(0, 0, 0, 0), tint_col = ImVec4(1, 1, 1, 1)
CIMGUI_API void ImGuiImageWithBgEx(ImTextureRef tex_ref, ImVec2 image_size, ImVec2 uv0 /* = ImVec2(0, 0) */, ImVec2 uv1 /* = ImVec2(1, 1) */, ImVec4 bg_col /* = ImVec4(0, 0, 0, 0) */, ImVec4 tint_col /* = ImVec4(1, 1, 1, 1) */);
CIMGUI_API bool ImGuiImageButton(const char* str_id, ImTextureRef tex_ref, ImVec2 image_size);                                             // Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1), bg_col = ImVec4(0, 0, 0, 0), tint_col = ImVec4(1, 1, 1, 1)
CIMGUI_API bool ImGuiImageButtonEx(const char* str_id, ImTextureRef tex_ref, ImVec2 image_size, ImVec2 uv0 /* = ImVec2(0, 0) */, ImVec2 uv1 /* = ImVec2(1, 1) */, ImVec4 bg_col /* = ImVec4(0, 0, 0, 0) */, ImVec4 tint_col /* = ImVec4(1, 1, 1, 1) */);

// Widgets: Combo Box (Dropdown)
// - The BeginCombo()/EndCombo() api allows you to manage your contents and selection state however you want it, by creating e.g. Selectable() items.
// - The old Combo() api are helpers over BeginCombo()/EndCombo() which are kept available for convenience purpose. This is analogous to how ListBox are created.
CIMGUI_API bool ImGuiBeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags /* = 0 */);
CIMGUI_API void ImGuiEndCombo(void);                                                                                                                           // only call EndCombo() if BeginCombo() returns true!
CIMGUI_API bool ImGuiComboChar(const char* label, int* current_item, const char*const items[], int items_count);                                               // Implied popup_max_height_in_items = -1
CIMGUI_API bool ImGuiComboCharEx(const char* label, int* current_item, const char*const items[], int items_count, int popup_max_height_in_items /* = -1 */);
CIMGUI_API bool ImGuiCombo(const char* label, int* current_item, const char* items_separated_by_zeros);                                                        // Implied popup_max_height_in_items = -1
CIMGUI_API bool ImGuiComboEx(const char* label, int* current_item, const char* items_separated_by_zeros, int popup_max_height_in_items /* = -1 */);            // Separate items with \0 within a string, end item-list with \0\0. e.g. "One\0Two\0Three\0"
CIMGUI_API bool ImGuiComboCallback(const char* label, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data, int items_count);   // Implied popup_max_height_in_items = -1
CIMGUI_API bool ImGuiComboCallbackEx(const char* label, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data, int items_count, int popup_max_height_in_items /* = -1 */);
...
CIMGUI_API bool ImGuiDragFloat(const char* label, float* v);                                                  // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
CIMGUI_API bool ImGuiDragFloatEx(const char* label, float* v, float v_speed /* = 1.0f */, float v_min /* = 0.0f */, float v_max /* = 0.0f */, const char* format /* = "%.3f" */, ImGuiSliderFlags flags /* = 0 */); // If v_min >= v_max we have no bound
CIMGUI_API bool ImGuiDragFloat2(const char* label, float v[2]);                                               // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
CIMGUI_API bool ImGuiDragFloat2Ex(const char* label, float v[2], float v_speed /* = 1.0f */, float v_min /* = 0.0f */, float v_max /* = 0.0f */, const char* format /* = "%.3f" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiDragFloat3(const char* label, float v[3]);                                               // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
CIMGUI_API bool ImGuiDragFloat3Ex(const char* label, float v[3], float v_speed /* = 1.0f */, float v_min /* = 0.0f */, float v_max /* = 0.0f */, const char* format /* = "%.3f" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiDragFloat4(const char* label, float v[4]);                                               // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
CIMGUI_API bool ImGuiDragFloat4Ex(const char* label, float v[4], float v_speed /* = 1.0f */, float v_min /* = 0.0f */, float v_max /* = 0.0f */, const char* format /* = "%.3f" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiDragFloatRange2(const char* label, float* v_current_min, float* v_current_max);          // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", format_max = NULL, flags = 0
CIMGUI_API bool ImGuiDragFloatRange2Ex(const char* label, float* v_current_min, float* v_current_max, float v_speed /* = 1.0f */, float v_min /* = 0.0f */, float v_max /* = 0.0f */, const char* format /* = "%.3f" */, const char* format_max /* = NULL */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiDragInt(const char* label, int* v);                                                      // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
CIMGUI_API bool ImGuiDragIntEx(const char* label, int* v, float v_speed /* = 1.0f */, int v_min /* = 0 */, int v_max /* = 0 */, const char* format /* = "%d" */, ImGuiSliderFlags flags /* = 0 */); // If v_min >= v_max we have no bound
CIMGUI_API bool ImGuiDragInt2(const char* label, int v[2]);                                                   // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
CIMGUI_API bool ImGuiDragInt2Ex(const char* label, int v[2], float v_speed /* = 1.0f */, int v_min /* = 0 */, int v_max /* = 0 */, const char* format /* = "%d" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiDragInt3(const char* label, int v[3]);                                                   // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
CIMGUI_API bool ImGuiDragInt3Ex(const char* label, int v[3], float v_speed /* = 1.0f */, int v_min /* = 0 */, int v_max /* = 0 */, const char* format /* = "%d" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiDragInt4(const char* label, int v[4]);                                                   // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
CIMGUI_API bool ImGuiDragInt4Ex(const char* label, int v[4], float v_speed /* = 1.0f */, int v_min /* = 0 */, int v_max /* = 0 */, const char* format /* = "%d" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiDragIntRange2(const char* label, int* v_current_min, int* v_current_max);                // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", format_max = NULL, flags = 0
CIMGUI_API bool ImGuiDragIntRange2Ex(const char* label, int* v_current_min, int* v_current_max, float v_speed /* = 1.0f */, int v_min /* = 0 */, int v_max /* = 0 */, const char* format /* = "%d" */, const char* format_max /* = NULL */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiDragScalar(const char* label, ImGuiDataType data_type, void* p_data);                    // Implied v_speed = 1.0f, p_min = NULL, p_max = NULL, format = NULL, flags = 0
CIMGUI_API bool ImGuiDragScalarEx(const char* label, ImGuiDataType data_type, void* p_data, float v_speed /* = 1.0f */, const void* p_min /* = NULL */, const void* p_max /* = NULL */, const char* format /* = NULL */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiDragScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components);   // Implied v_speed = 1.0f, p_min = NULL, p_max = NULL, format = NULL, flags = 0
CIMGUI_API bool ImGuiDragScalarNEx(const char* label, ImGuiDataType data_type, void* p_data, int components, float v_speed /* = 1.0f */, const void* p_min /* = NULL */, const void* p_max /* = NULL */, const char* format /* = NULL */, ImGuiSliderFlags flags /* = 0 */);
...
//   If you get a warning converting a float to ImGuiSliderFlags, read https://github.com/ocornut/imgui/issues/3361
CIMGUI_API bool ImGuiSliderFloat(const char* label, float* v, float v_min, float v_max);                                                                                        // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGuiSliderFloatEx(const char* label, float* v, float v_min, float v_max, const char* format /* = "%.3f" */, ImGuiSliderFlags flags /* = 0 */);                 // adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit display.
CIMGUI_API bool ImGuiSliderFloat2(const char* label, float v[2], float v_min, float v_max);                                                                                     // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGuiSliderFloat2Ex(const char* label, float v[2], float v_min, float v_max, const char* format /* = "%.3f" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiSliderFloat3(const char* label, float v[3], float v_min, float v_max);                                                                                     // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGuiSliderFloat3Ex(const char* label, float v[3], float v_min, float v_max, const char* format /* = "%.3f" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiSliderFloat4(const char* label, float v[4], float v_min, float v_max);                                                                                     // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGuiSliderFloat4Ex(const char* label, float v[4], float v_min, float v_max, const char* format /* = "%.3f" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiSliderAngle(const char* label, float* v_rad);                                                                                                              // Implied v_degrees_min = -360.0f, v_degrees_max = +360.0f, format = "%.0f deg", flags = 0
CIMGUI_API bool ImGuiSliderAngleEx(const char* label, float* v_rad, float v_degrees_min /* = -360.0f */, float v_degrees_max /* = +360.0f */, const char* format /* = "%.0f deg" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiSliderInt(const char* label, int* v, int v_min, int v_max);                                                                                                // Implied format = "%d", flags = 0
CIMGUI_API bool ImGuiSliderIntEx(const char* label, int* v, int v_min, int v_max, const char* format /* = "%d" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiSliderInt2(const char* label, int v[2], int v_min, int v_max);                                                                                             // Implied format = "%d", flags = 0
CIMGUI_API bool ImGuiSliderInt2Ex(const char* label, int v[2], int v_min, int v_max, const char* format /* = "%d" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiSliderInt3(const char* label, int v[3], int v_min, int v_max);                                                                                             // Implied format = "%d", flags = 0
CIMGUI_API bool ImGuiSliderInt3Ex(const char* label, int v[3], int v_min, int v_max, const char* format /* = "%d" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiSliderInt4(const char* label, int v[4], int v_min, int v_max);                                                                                             // Implied format = "%d", flags = 0
CIMGUI_API bool ImGuiSliderInt4Ex(const char* label, int v[4], int v_min, int v_max, const char* format /* = "%d" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiSliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max);                                              // Implied format = NULL, flags = 0
CIMGUI_API bool ImGuiSliderScalarEx(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format /* = NULL */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiSliderScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, const void* p_min, const void* p_max);                             // Implied format = NULL, flags = 0
CIMGUI_API bool ImGuiSliderScalarNEx(const char* label, ImGuiDataType data_type, void* p_data, int components, const void* p_min, const void* p_max, const char* format /* = NULL */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiVSliderFloat(const char* label, ImVec2 size, float* v, float v_min, float v_max);                                                                          // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGuiVSliderFloatEx(const char* label, ImVec2 size, float* v, float v_min, float v_max, const char* format /* = "%.3f" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiVSliderInt(const char* label, ImVec2 size, int* v, int v_min, int v_max);                                                                                  // Implied format = "%d", flags = 0
CIMGUI_API bool ImGuiVSliderIntEx(const char* label, ImVec2 size, int* v, int v_min, int v_max, const char* format /* = "%d" */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGuiVSliderScalar(const char* label, ImVec2 size, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max);                                // Implied format = NULL, flags = 0
CIMGUI_API bool ImGuiVSliderScalarEx(const char* label, ImVec2 size, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format /* = NULL */, ImGuiSliderFlags flags /* = 0 */);
...
CIMGUI_API bool ImGuiInputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags /* = 0 */);                               // Implied callback = NULL, user_data = NULL
CIMGUI_API bool ImGuiInputTextEx(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags /* = 0 */, ImGuiInputTextCallback callback /* = NULL */, void* user_data /* = NULL */);
CIMGUI_API bool ImGuiInputTextMultiline(const char* label, char* buf, size_t buf_size);                                                           // Implied size = ImVec2(0, 0), flags = 0, callback = NULL, user_data = NULL
CIMGUI_API bool ImGuiInputTextMultilineEx(const char* label, char* buf, size_t buf_size, ImVec2 size /* = ImVec2(0, 0) */, ImGuiInputTextFlags flags /* = 0 */, ImGuiInputTextCallback callback /* = NULL */, void* user_data /* = NULL */);
CIMGUI_API bool ImGuiInputTextWithHint(const char* label, const char* hint, char* buf, size_t buf_size, ImGuiInputTextFlags flags /* = 0 */);     // Implied callback = NULL, user_data = NULL
CIMGUI_API bool ImGuiInputTextWithHintEx(const char* label, const char* hint, char* buf, size_t buf_size, ImGuiInputTextFlags flags /* = 0 */, ImGuiInputTextCallback callback /* = NULL */, void* user_data /* = NULL */);
CIMGUI_API bool ImGuiInputFloat(const char* label, float* v);                                                                                     // Implied step = 0.0f, step_fast = 0.0f, format = "%.3f", flags = 0
CIMGUI_API bool ImGuiInputFloatEx(const char* label, float* v, float step /* = 0.0f */, float step_fast /* = 0.0f */, const char* format /* = "%.3f" */, ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool ImGuiInputFloat2(const char* label, float v[2]);                                                                                  // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGuiInputFloat2Ex(const char* label, float v[2], const char* format /* = "%.3f" */, ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool ImGuiInputFloat3(const char* label, float v[3]);                                                                                  // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGuiInputFloat3Ex(const char* label, float v[3], const char* format /* = "%.3f" */, ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool ImGuiInputFloat4(const char* label, float v[4]);                                                                                  // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGuiInputFloat4Ex(const char* label, float v[4], const char* format /* = "%.3f" */, ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool ImGuiInputInt(const char* label, int* v);                                                                                         // Implied step = 1, step_fast = 100, flags = 0
CIMGUI_API bool ImGuiInputIntEx(const char* label, int* v, int step /* = 1 */, int step_fast /* = 100 */, ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool ImGuiInputInt2(const char* label, int v[2], ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool ImGuiInputInt3(const char* label, int v[3], ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool ImGuiInputInt4(const char* label, int v[4], ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool ImGuiInputDouble(const char* label, double* v);                                                                                   // Implied step = 0.0, step_fast = 0.0, format = "%.6f", flags = 0
CIMGUI_API bool ImGuiInputDoubleEx(const char* label, double* v, double step /* = 0.0 */, double step_fast /* = 0.0 */, const char* format /* = "%.6f" */, ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool ImGuiInputScalar(const char* label, ImGuiDataType data_type, void* p_data);                                                       // Implied p_step = NULL, p_step_fast = NULL, format = NULL, flags = 0
CIMGUI_API bool ImGuiInputScalarEx(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step /* = NULL */, const void* p_step_fast /* = NULL */, const char* format /* = NULL */, ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool ImGuiInputScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components);                                      // Implied p_step = NULL, p_step_fast = NULL, format = NULL, flags = 0
CIMGUI_API bool ImGuiInputScalarNEx(const char* label, ImGuiDataType data_type, void* p_data, int components, const void* p_step /* = NULL */, const void* p_step_fast /* = NULL */, const char* format /* = NULL */, ImGuiInputTextFlags flags /* = 0 */);

// Widgets: Color Editor/Picker (tip: the ColorEdit* functions have a little color square that can be left-clicked to open a picker, and right-clicked to open an option menu.)
// - Note that in C++ a 'float v[X]' function argument is the _same_ as 'float* v', the array syntax is just a way to document the number of elements that are expected to be accessible.
// - You can pass the address of a first float element out of a contiguous structure, e.g. &myvector.x
CIMGUI_API bool ImGuiColorEdit3(const char* label, float col[3], ImGuiColorEditFlags flags /* = 0 */);
CIMGUI_API bool ImGuiColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags /* = 0 */);
CIMGUI_API bool ImGuiColorPicker3(const char* label, float col[3], ImGuiColorEditFlags flags /* = 0 */);
CIMGUI_API bool ImGuiColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags /* = 0 */, const float* ref_col /* = NULL */);
CIMGUI_API bool ImGuiColorButton(const char* desc_id, ImVec4 col, ImGuiColorEditFlags flags /* = 0 */);                                       // Implied size = ImVec2(0, 0)
CIMGUI_API bool ImGuiColorButtonEx(const char* desc_id, ImVec4 col, ImGuiColorEditFlags flags /* = 0 */, ImVec2 size /* = ImVec2(0, 0) */);   // display a color square/button, hover for details, return true when pressed.
CIMGUI_API void ImGuiSetColorEditOptions(ImGuiColorEditFlags flags);                                                                          // initialize current options (generally on application startup) if you want to select a default format, picker type, etc. User will be able to change many settings, unless you pass the _NoOptions flag to your calls.

// Widgets: Trees
// - TreeNode functions return true when the node is open, in which case you need to also call TreePop() when you are finished displaying the tree node contents.
CIMGUI_API bool  ImGuiTreeNode(const char* label);
CIMGUI_API bool  ImGuiTreeNodeStr(const char* str_id, const char* fmt, ...) IM_FMTARGS(2);                                         // helper variation to easily decorrelate the id from the displayed string. Read the FAQ about why and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
CIMGUI_API bool  ImGuiTreeNodeStrUnformatted(const char* str_id, const char* text);                                                // helper variation to easily decorrelate the id from the displayed string. Read the FAQ about why and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
CIMGUI_API bool  ImGuiTreeNodePtr(const void* ptr_id, const char* fmt, ...) IM_FMTARGS(2);                                         // "
CIMGUI_API bool  ImGuiTreeNodePtrUnformatted(const void* ptr_id, const char* text);                                                // "
CIMGUI_API bool  ImGuiTreeNodeV(const char* str_id, const char* fmt, va_list args) IM_FMTLIST(2);
CIMGUI_API bool  ImGuiTreeNodeVPtr(const void* ptr_id, const char* fmt, va_list args) IM_FMTLIST(2);
CIMGUI_API bool  ImGuiTreeNodeEx(const char* label, ImGuiTreeNodeFlags flags /* = 0 */);
CIMGUI_API bool  ImGuiTreeNodeExStr(const char* str_id, ImGuiTreeNodeFlags flags, const char* fmt, ...) IM_FMTARGS(3);
CIMGUI_API bool  ImGuiTreeNodeExStrUnformatted(const char* str_id, ImGuiTreeNodeFlags flags, const char* text);
CIMGUI_API bool  ImGuiTreeNodeExPtr(const void* ptr_id, ImGuiTreeNodeFlags flags, const char* fmt, ...) IM_FMTARGS(3);
CIMGUI_API bool  ImGuiTreeNodeExPtrUnformatted(const void* ptr_id, ImGuiTreeNodeFlags flags, const char* text);
CIMGUI_API bool  ImGuiTreeNodeExV(const char* str_id, ImGuiTreeNodeFlags flags, const char* fmt, va_list args) IM_FMTLIST(3);
CIMGUI_API bool  ImGuiTreeNodeExVPtr(const void* ptr_id, ImGuiTreeNodeFlags flags, const char* fmt, va_list args) IM_FMTLIST(3);
CIMGUI_API void  ImGuiTreePush(const char* str_id);                                                                                // ~ Indent()+PushID(). Already called by TreeNode() when returning true, but you can call TreePush/TreePop yourself if desired.
CIMGUI_API void  ImGuiTreePushPtr(const void* ptr_id);                                                                             // "
CIMGUI_API void  ImGuiTreePop(void);                                                                                               // ~ Unindent()+PopID()
CIMGUI_API float ImGuiGetTreeNodeToLabelSpacing(void);                                                                             // horizontal distance preceding label when using TreeNode*() or Bullet() == (g.FontSize + style.FramePadding.x*2) for a regular unframed TreeNode
CIMGUI_API bool  ImGuiCollapsingHeader(const char* label, ImGuiTreeNodeFlags flags /* = 0 */);                                     // if returning 'true' the header is open. doesn't indent nor push on ID stack. user doesn't have to call TreePop().
CIMGUI_API bool  ImGuiCollapsingHeaderBoolPtr(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags /* = 0 */);             // when 'p_visible != NULL': if '*p_visible==true' display an additional small close button on upper right of the header which will set the bool to false when clicked, if '*p_visible==false' don't display the header.
CIMGUI_API void  ImGuiSetNextItemOpen(bool is_open, ImGuiCond cond /* = 0 */);                                                     // set next TreeNode/CollapsingHeader open state.
CIMGUI_API void  ImGuiSetNextItemStorageID(ImGuiID storage_id);                                                                    // set id to use for open/close storage (default to same as item id).

// Widgets: Selectables
// - A selectable highlights when hovered, and can display another color when selected.
// - Neighbors selectable extend their highlight bounds in order to leave no gap between them. This is so a series of selected Selectable appear contiguous.
CIMGUI_API bool ImGuiSelectable(const char* label);                                                                                                          // Implied selected = false, flags = 0, size = ImVec2(0, 0)
CIMGUI_API bool ImGuiSelectableEx(const char* label, bool selected /* = false */, ImGuiSelectableFlags flags /* = 0 */, ImVec2 size /* = ImVec2(0, 0) */);   // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so you can modify your selection state. size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
CIMGUI_API bool ImGuiSelectableBoolPtr(const char* label, bool* p_selected, ImGuiSelectableFlags flags /* = 0 */);                                           // Implied size = ImVec2(0, 0)
CIMGUI_API bool ImGuiSelectableBoolPtrEx(const char* label, bool* p_selected, ImGuiSelectableFlags flags /* = 0 */, ImVec2 size /* = ImVec2(0, 0) */);       // "bool* p_selected" point to the selection state (read-write), as a convenient helper.
...
CIMGUI_API ImGuiMultiSelectIO* ImGuiBeginMultiSelect(ImGuiMultiSelectFlags flags);                              // Implied selection_size = -1, items_count = -1
CIMGUI_API ImGuiMultiSelectIO* ImGuiBeginMultiSelectEx(ImGuiMultiSelectFlags flags, int selection_size /* = -1 */, int items_count /* = -1 */);
CIMGUI_API ImGuiMultiSelectIO* ImGuiEndMultiSelect(void);
CIMGUI_API void                ImGuiSetNextItemSelectionUserData(ImGuiSelectionUserData selection_user_data);
CIMGUI_API bool                ImGuiIsItemToggledSelection(void);                                               // Was the last item selection state toggled? Useful if you need the per-item information _before_ reaching EndMultiSelect(). We only returns toggle _event_ in order to handle clipping correctly.
...
CIMGUI_API bool ImGuiBeginListBox(const char* label, ImVec2 size /* = ImVec2(0, 0) */);                                                                          // open a framed scrolling region
CIMGUI_API void ImGuiEndListBox(void);                                                                                                                           // only call EndListBox() if BeginListBox() returned true!
CIMGUI_API bool ImGuiListBox(const char* label, int* current_item, const char*const items[], int items_count, int height_in_items /* = -1 */);
CIMGUI_API bool ImGuiListBoxCallback(const char* label, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data, int items_count);   // Implied height_in_items = -1
CIMGUI_API bool ImGuiListBoxCallbackEx(const char* label, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data, int items_count, int height_in_items /* = -1 */);

// Widgets: Data Plotting
// - Consider using ImPlot (https://github.com/epezent/implot) which is much better!
CIMGUI_API void ImGuiPlotLines(const char* label, const float* values, int values_count);                                                   // Implied values_offset = 0, overlay_text = NULL, scale_min = FLT_MAX, scale_max = FLT_MAX, graph_size = ImVec2(0, 0), stride = sizeof(float)
CIMGUI_API void ImGuiPlotLinesEx(const char* label, const float* values, int values_count, int values_offset /* = 0 */, const char* overlay_text /* = NULL */, float scale_min /* = FLT_MAX */, float scale_max /* = FLT_MAX */, ImVec2 graph_size /* = ImVec2(0, 0) */, int stride /* = sizeof(float) */);
CIMGUI_API void ImGuiPlotLinesCallback(const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count);       // Implied values_offset = 0, overlay_text = NULL, scale_min = FLT_MAX, scale_max = FLT_MAX, graph_size = ImVec2(0, 0)
CIMGUI_API void ImGuiPlotLinesCallbackEx(const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset /* = 0 */, const char* overlay_text /* = NULL */, float scale_min /* = FLT_MAX */, float scale_max /* = FLT_MAX */, ImVec2 graph_size /* = ImVec2(0, 0) */);
CIMGUI_API void ImGuiPlotHistogram(const char* label, const float* values, int values_count);                                               // Implied values_offset = 0, overlay_text = NULL, scale_min = FLT_MAX, scale_max = FLT_MAX, graph_size = ImVec2(0, 0), stride = sizeof(float)
CIMGUI_API void ImGuiPlotHistogramEx(const char* label, const float* values, int values_count, int values_offset /* = 0 */, const char* overlay_text /* = NULL */, float scale_min /* = FLT_MAX */, float scale_max /* = FLT_MAX */, ImVec2 graph_size /* = ImVec2(0, 0) */, int stride /* = sizeof(float) */);
CIMGUI_API void ImGuiPlotHistogramCallback(const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count);   // Implied values_offset = 0, overlay_text = NULL, scale_min = FLT_MAX, scale_max = FLT_MAX, graph_size = ImVec2(0, 0)
CIMGUI_API void ImGuiPlotHistogramCallbackEx(const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset /* = 0 */, const char* overlay_text /* = NULL */, float scale_min /* = FLT_MAX */, float scale_max /* = FLT_MAX */, ImVec2 graph_size /* = ImVec2(0, 0) */);
...
CIMGUI_API bool ImGuiBeginMenuBar(void);                                          // append to menu-bar of current window (requires ImGuiWindowFlags_MenuBar flag set on parent window).
CIMGUI_API void ImGuiEndMenuBar(void);                                            // only call EndMenuBar() if BeginMenuBar() returns true!
CIMGUI_API bool ImGuiBeginMainMenuBar(void);                                      // create and append to a full screen menu-bar.
CIMGUI_API void ImGuiEndMainMenuBar(void);                                        // only call EndMainMenuBar() if BeginMainMenuBar() returns true!
CIMGUI_API bool ImGuiBeginMenu(const char* label);                                // Implied enabled = true
CIMGUI_API bool ImGuiBeginMenuEx(const char* label, bool enabled /* = true */);   // create a sub-menu entry. only call EndMenu() if this returns true!
CIMGUI_API void ImGuiEndMenu(void);                                               // only call EndMenu() if BeginMenu() returns true!
CIMGUI_API bool ImGuiMenuItem(const char* label);                                 // Implied shortcut = NULL, selected = false, enabled = true
CIMGUI_API bool ImGuiMenuItemEx(const char* label, const char* shortcut /* = NULL */, bool selected /* = false */, bool enabled /* = true */); // return true when activated.
CIMGUI_API bool ImGuiMenuItemBoolPtr(const char* label, const char* shortcut, bool* p_selected, bool enabled /* = true */); // return true when activated + toggle (*p_selected) if p_selected != NULL

// Tooltips
// - Tooltips are windows following the mouse. They do not take focus away.
// - A tooltip window can contain items of any types.
// - SetTooltip() is more or less a shortcut for the 'if (BeginTooltip()) { Text(...); EndTooltip(); }' idiom (with a subtlety that it discard any previously submitted tooltip)
CIMGUI_API bool ImGuiBeginTooltip(void);                                         // begin/append a tooltip window.
CIMGUI_API void ImGuiEndTooltip(void);                                           // only call EndTooltip() if BeginTooltip()/BeginItemTooltip() returns true!
CIMGUI_API void ImGuiSetTooltip(const char* fmt, ...) IM_FMTARGS(1);             // set a text-only tooltip. Often used after a ImGui::IsItemHovered() check. Override any previous call to SetTooltip().
CIMGUI_API void ImGuiSetTooltipUnformatted(const char* text);                    // set a text-only tooltip. Often used after a ImGui::IsItemHovered() check. Override any previous call to SetTooltip().
CIMGUI_API void ImGuiSetTooltipV(const char* fmt, va_list args) IM_FMTLIST(1);

// Tooltips: helpers for showing a tooltip when hovering an item
// - BeginItemTooltip() is a shortcut for the 'if (IsItemHovered(ImGuiHoveredFlags_ForTooltip) && BeginTooltip())' idiom.
// - SetItemTooltip() is a shortcut for the 'if (IsItemHovered(ImGuiHoveredFlags_ForTooltip)) { SetTooltip(...); }' idiom.
// - Where 'ImGuiHoveredFlags_ForTooltip' itself is a shortcut to use 'style.HoverFlagsForTooltipMouse' or 'style.HoverFlagsForTooltipNav' depending on active input type. For mouse it defaults to 'ImGuiHoveredFlags_Stationary | ImGuiHoveredFlags_DelayShort'.
CIMGUI_API bool ImGuiBeginItemTooltip(void);                                         // begin/append a tooltip window if preceding item was hovered.
CIMGUI_API void ImGuiSetItemTooltip(const char* fmt, ...) IM_FMTARGS(1);             // set a text-only tooltip if preceding item was hovered. override any previous call to SetTooltip().
CIMGUI_API void ImGuiSetItemTooltipUnformatted(const char* text);                    // set a text-only tooltip if preceding item was hovered. override any previous call to SetTooltip().
CIMGUI_API void ImGuiSetItemTooltipV(const char* fmt, va_list args) IM_FMTLIST(1);
...
CIMGUI_API bool ImGuiBeginPopup(const char* str_id, ImGuiWindowFlags flags /* = 0 */);                                 // return true if the popup is open, and you can start outputting to it.
CIMGUI_API bool ImGuiBeginPopupModal(const char* name, bool* p_open /* = NULL */, ImGuiWindowFlags flags /* = 0 */);   // return true if the modal is open, and you can start outputting to it.
CIMGUI_API void ImGuiEndPopup(void);                                                                                   // only call EndPopup() if BeginPopupXXX() returns true!
...
CIMGUI_API void ImGuiOpenPopup(const char* str_id, ImGuiPopupFlags popup_flags /* = 0 */);                           // call to mark popup as open (don't call every frame!).
CIMGUI_API void ImGuiOpenPopupID(ImGuiID id, ImGuiPopupFlags popup_flags /* = 0 */);                                 // id overload to facilitate calling from nested stacks
CIMGUI_API void ImGuiOpenPopupOnItemClick(const char* str_id /* = NULL */, ImGuiPopupFlags popup_flags /* = 1 */);   // helper to open popup when clicked on last item. Default to ImGuiPopupFlags_MouseButtonRight == 1. (note: actually triggers on the mouse _released_ event to be consistent with popup behaviors)
CIMGUI_API void ImGuiCloseCurrentPopup(void);                                                                        // manually close the popup we have begin-ed into.
...
CIMGUI_API bool ImGuiBeginPopupContextItem(void);                                                                         // Implied str_id = NULL, popup_flags = 1
CIMGUI_API bool ImGuiBeginPopupContextItemEx(const char* str_id /* = NULL */, ImGuiPopupFlags popup_flags /* = 1 */);     // open+begin popup when clicked on last item. Use str_id==NULL to associate the popup to previous item. If you want to use that on a non-interactive item such as Text() you need to pass in an explicit ID here. read comments in .cpp!
CIMGUI_API bool ImGuiBeginPopupContextWindow(void);                                                                       // Implied str_id = NULL, popup_flags = 1
CIMGUI_API bool ImGuiBeginPopupContextWindowEx(const char* str_id /* = NULL */, ImGuiPopupFlags popup_flags /* = 1 */);   // open+begin popup when clicked on current window.
CIMGUI_API bool ImGuiBeginPopupContextVoid(void);                                                                         // Implied str_id = NULL, popup_flags = 1
CIMGUI_API bool ImGuiBeginPopupContextVoidEx(const char* str_id /* = NULL */, ImGuiPopupFlags popup_flags /* = 1 */);     // open+begin popup when clicked in void (where there are no windows).

// Popups: query functions
//  - IsPopupOpen(): return true if the popup is open at the current BeginPopup() level of the popup stack.
//  - IsPopupOpen() with ImGuiPopupFlags_AnyPopupId: return true if any popup is open at the current BeginPopup() level of the popup stack.
//  - IsPopupOpen() with ImGuiPopupFlags_AnyPopupId + ImGuiPopupFlags_AnyPopupLevel: return true if any popup is open.
CIMGUI_API bool ImGuiIsPopupOpen(const char* str_id, ImGuiPopupFlags flags /* = 0 */);   // return true if the popup is open.
...
// - 5. Call EndTable()
CIMGUI_API bool ImGuiBeginTable(const char* str_id, int columns, ImGuiTableFlags flags /* = 0 */);                // Implied outer_size = ImVec2(0.0f, 0.0f), inner_width = 0.0f
CIMGUI_API bool ImGuiBeginTableEx(const char* str_id, int columns, ImGuiTableFlags flags /* = 0 */, ImVec2 outer_size /* = ImVec2(0.0f, 0.0f) */, float inner_width /* = 0.0f */);
CIMGUI_API void ImGuiEndTable(void);                                                                              // only call EndTable() if BeginTable() returns true!
CIMGUI_API void ImGuiTableNextRow(void);                                                                          // Implied row_flags = 0, min_row_height = 0.0f
CIMGUI_API void ImGuiTableNextRowEx(ImGuiTableRowFlags row_flags /* = 0 */, float min_row_height /* = 0.0f */);   // append into the first cell of a new row.
CIMGUI_API bool ImGuiTableNextColumn(void);                                                                       // append into the next column (or first column of next row if currently in last column). Return true when column is visible.
CIMGUI_API bool ImGuiTableSetColumnIndex(int column_n);                                                           // append into the specified column. Return true when column is visible.
...
CIMGUI_API void ImGuiTableSetupColumn(const char* label, ImGuiTableColumnFlags flags /* = 0 */);   // Implied init_width_or_weight = 0.0f, user_id = 0
CIMGUI_API void ImGuiTableSetupColumnEx(const char* label, ImGuiTableColumnFlags flags /* = 0 */, float init_width_or_weight /* = 0.0f */, ImGuiID user_id /* = 0 */);
CIMGUI_API void ImGuiTableSetupScrollFreeze(int cols, int rows);                                   // lock columns/rows so they stay visible when scrolled.
CIMGUI_API void ImGuiTableHeader(const char* label);                                               // submit one header cell manually (rarely used)
CIMGUI_API void ImGuiTableHeadersRow(void);                                                        // submit a row with headers cells based on data provided to TableSetupColumn() + submit context menu
CIMGUI_API void ImGuiTableAngledHeadersRow(void);                                                  // submit a row with angled headers for every column with the ImGuiTableColumnFlags_AngledHeader flag. MUST BE FIRST ROW.
...
CIMGUI_API ImGuiTableSortSpecs*  ImGuiTableGetSortSpecs(void);                        // get latest sort specs for the table (NULL if not sorting).  Lifetime: don't hold on this pointer over multiple frames or past any subsequent call to BeginTable().
CIMGUI_API int                   ImGuiTableGetColumnCount(void);                      // return number of columns (value passed to BeginTable)
CIMGUI_API int                   ImGuiTableGetColumnIndex(void);                      // return current column index.
CIMGUI_API int                   ImGuiTableGetRowIndex(void);                         // return current row index (header rows are accounted for)
CIMGUI_API const char*           ImGuiTableGetColumnName(int column_n /* = -1 */);    // return "" if column didn't have a name declared by TableSetupColumn(). Pass -1 to use current column.
CIMGUI_API ImGuiTableColumnFlags ImGuiTableGetColumnFlags(int column_n /* = -1 */);   // return column flags so you can query their Enabled/Visible/Sorted/Hovered status flags. Pass -1 to use current column.
CIMGUI_API void                  ImGuiTableSetColumnEnabled(int column_n, bool v);    // change user accessible enabled/disabled state of a column. Set to false to hide the column. User can use the context menu to change this themselves (right-click in headers, or right-click in columns body with ImGuiTableFlags_ContextMenuInBody)
CIMGUI_API int                   ImGuiTableGetHoveredColumn(void);                    // return hovered column. return -1 when table is not hovered. return columns_count if the unused space at the right of visible columns is hovered. Can also use (TableGetColumnFlags() & ImGuiTableColumnFlags_IsHovered) instead.
CIMGUI_API void                  ImGuiTableSetBgColor(ImGuiTableBgTarget target, ImU32 color, int column_n /* = -1 */); // change the color of a cell, row, or column. See ImGuiTableBgTarget_ flags for details.

// Legacy Columns API (prefer using Tables!)
// - You can also use SameLine(pos_x) to mimic simplified columns.
CIMGUI_API void  ImGuiColumns(void);                                       // Implied count = 1, id = NULL, borders = true
CIMGUI_API void  ImGuiColumnsEx(int count /* = 1 */, const char* id /* = NULL */, bool borders /* = true */);
CIMGUI_API void  ImGuiNextColumn(void);                                    // next column, defaults to current row or next row if the current row is finished
CIMGUI_API int   ImGuiGetColumnIndex(void);                                // get current column index
CIMGUI_API float ImGuiGetColumnWidth(int column_index /* = -1 */);         // get column width (in pixels). pass -1 to use current column
CIMGUI_API void  ImGuiSetColumnWidth(int column_index, float width);       // set column width (in pixels). pass -1 to use current column
CIMGUI_API float ImGuiGetColumnOffset(int column_index /* = -1 */);        // get position of column line (in pixels, from the left side of the contents region). pass -1 to use current column, otherwise 0..GetColumnsCount() inclusive. column 0 is typically 0.0f
CIMGUI_API void  ImGuiSetColumnOffset(int column_index, float offset_x);   // set position of column line (in pixels, from the left side of the contents region). pass -1 to use current column
CIMGUI_API int   ImGuiGetColumnsCount(void);

// Tab Bars, Tabs
// - Note: Tabs are automatically created by the docking system (when in 'docking' branch). Use this to create tab bars/tabs yourself.
CIMGUI_API bool ImGuiBeginTabBar(const char* str_id, ImGuiTabBarFlags flags /* = 0 */);     // create and append into a TabBar
CIMGUI_API void ImGuiEndTabBar(void);                                                       // only call EndTabBar() if BeginTabBar() returns true!
CIMGUI_API bool ImGuiBeginTabItem(const char* label, bool* p_open /* = NULL */, ImGuiTabItemFlags flags /* = 0 */); // create a Tab. Returns true if the Tab is selected.
CIMGUI_API void ImGuiEndTabItem(void);                                                      // only call EndTabItem() if BeginTabItem() returns true!
CIMGUI_API bool ImGuiTabItemButton(const char* label, ImGuiTabItemFlags flags /* = 0 */);   // create a Tab behaving like a button. return true when clicked. cannot be selected in the tab bar.
CIMGUI_API void ImGuiSetTabItemClosed(const char* tab_or_docked_window_label);              // notify TabBar or Docking system of a closed tab/window ahead (useful to reduce visual flicker on reorderable tab bars). For tab-bar: call after BeginTabBar() and before Tab submissions. Otherwise call with a window name.

// Logging/Capture
// - All text output from the interface can be captured into tty/file/clipboard. By default, tree nodes are automatically opened during logging.
CIMGUI_API void ImGuiLogToTTY(int auto_open_depth /* = -1 */);                                       // start logging to tty (stdout)
CIMGUI_API void ImGuiLogToFile(int auto_open_depth /* = -1 */, const char* filename /* = NULL */);   // start logging to file
CIMGUI_API void ImGuiLogToClipboard(int auto_open_depth /* = -1 */);                                 // start logging to OS clipboard
CIMGUI_API void ImGuiLogFinish(void);                                                                // stop logging (close file, etc.)
CIMGUI_API void ImGuiLogButtons(void);                                                               // helper to display buttons for logging to tty/file/clipboard
CIMGUI_API void ImGuiLogText(const char* fmt, ...) IM_FMTARGS(1);                                    // pass text data straight to log (without being displayed)
CIMGUI_API void ImGuiLogTextUnformatted(const char* text);                                           // pass text data straight to log (without being displayed)
CIMGUI_API void ImGuiLogTextV(const char* fmt, va_list args) IM_FMTLIST(1);
...
CIMGUI_API bool                ImGuiBeginDragDropSource(ImGuiDragDropFlags flags /* = 0 */);                       // call after submitting an item which may be dragged. when this return true, you can call SetDragDropPayload() + EndDragDropSource()
CIMGUI_API bool                ImGuiSetDragDropPayload(const char* type, const void* data, size_t sz, ImGuiCond cond /* = 0 */); // type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui internal types. Data is copied and held by imgui. Return true when payload has been accepted.
CIMGUI_API void                ImGuiEndDragDropSource(void);                                                       // only call EndDragDropSource() if BeginDragDropSource() returns true!
CIMGUI_API bool                ImGuiBeginDragDropTarget(void);                                                     // call after submitting an item that may receive a payload. If this returns true, you can call AcceptDragDropPayload() + EndDragDropTarget()
CIMGUI_API const ImGuiPayload* ImGuiAcceptDragDropPayload(const char* type, ImGuiDragDropFlags flags /* = 0 */);   // accept contents of a given type. If ImGuiDragDropFlags_AcceptBeforeDelivery is set you can peek into the payload before the mouse button is released.
CIMGUI_API void                ImGuiEndDragDropTarget(void);                                                       // only call EndDragDropTarget() if BeginDragDropTarget() returns true!
CIMGUI_API const ImGuiPayload* ImGuiGetDragDropPayload(void);                                                      // peek directly into the current payload from anywhere. returns NULL when drag and drop is finished or inactive. use ImGuiPayload::IsDataType() to test for the payload type.
...
CIMGUI_API void ImGuiBeginDisabled(bool disabled /* = true */);
CIMGUI_API void ImGuiEndDisabled(void);

// Clipping
// - Mouse hovering is affected by ImGui::PushClipRect() calls, unlike direct calls to ImDrawList::PushClipRect() which are render only.
CIMGUI_API void ImGuiPushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max, bool intersect_with_current_clip_rect);
CIMGUI_API void ImGuiPopClipRect(void);

// Focus, Activation
CIMGUI_API void ImGuiSetItemDefaultFocus(void);                      // make last item the default focused item of a newly appearing window.
CIMGUI_API void ImGuiSetKeyboardFocusHere(void);                     // Implied offset = 0
CIMGUI_API void ImGuiSetKeyboardFocusHereEx(int offset /* = 0 */);   // focus keyboard on the next widget. Use positive 'offset' to access sub components of a multiple component widget. Use -1 to access previous widget.

// Keyboard/Gamepad Navigation
CIMGUI_API void ImGuiSetNavCursorVisible(bool visible);   // alter visibility of keyboard/gamepad cursor. by default: show when using an arrow key, hide when clicking with mouse.

// Overlapping mode
CIMGUI_API void ImGuiSetNextItemAllowOverlap(void);   // allow next item to be overlapped by a subsequent item. Useful with invisible buttons, selectable, treenode covering an area where subsequent items may need to be added. Note that both Selectable() and TreeNode() have dedicated flags doing this.

// Item/Widgets Utilities and Query Functions
// - Most of the functions are referring to the previous Item that has been submitted.
// - See Demo Window under "Widgets->Querying Status" for an interactive visualization of most of those functions.
CIMGUI_API bool    ImGuiIsItemHovered(ImGuiHoveredFlags flags /* = 0 */);           // is the last item hovered? (and usable, aka not blocked by a popup, etc.). See ImGuiHoveredFlags for more options.
CIMGUI_API bool    ImGuiIsItemActive(void);                                         // is the last item active? (e.g. button being held, text field being edited. This will continuously return true while holding mouse button on an item. Items that don't interact will always return false)
CIMGUI_API bool    ImGuiIsItemFocused(void);                                        // is the last item focused for keyboard/gamepad navigation?
CIMGUI_API bool    ImGuiIsItemClicked(void);                                        // Implied mouse_button = 0
CIMGUI_API bool    ImGuiIsItemClickedEx(ImGuiMouseButton mouse_button /* = 0 */);   // is the last item hovered and mouse clicked on? (**)  == IsMouseClicked(mouse_button) && IsItemHovered()Important. (**) this is NOT equivalent to the behavior of e.g. Button(). Read comments in function definition.
CIMGUI_API bool    ImGuiIsItemVisible(void);                                        // is the last item visible? (items may be out of sight because of clipping/scrolling)
CIMGUI_API bool    ImGuiIsItemEdited(void);                                         // did the last item modify its underlying value this frame? or was pressed? This is generally the same as the "bool" return value of many widgets.
CIMGUI_API bool    ImGuiIsItemActivated(void);                                      // was the last item just made active (item was previously inactive).
CIMGUI_API bool    ImGuiIsItemDeactivated(void);                                    // was the last item just made inactive (item was previously active). Useful for Undo/Redo patterns with widgets that require continuous editing.
CIMGUI_API bool    ImGuiIsItemDeactivatedAfterEdit(void);                           // was the last item just made inactive and made a value change when it was active? (e.g. Slider/Drag moved). Useful for Undo/Redo patterns with widgets that require continuous editing. Note that you may get false positives (some widgets such as Combo()/ListBox()/Selectable() will return true even when clicking an already selected item).
CIMGUI_API bool    ImGuiIsItemToggledOpen(void);                                    // was the last item open state toggled? set by TreeNode().
CIMGUI_API bool    ImGuiIsAnyItemHovered(void);                                     // is any item hovered?
CIMGUI_API bool    ImGuiIsAnyItemActive(void);                                      // is any item active?
CIMGUI_API bool    ImGuiIsAnyItemFocused(void);                                     // is any item focused?
CIMGUI_API ImGuiID ImGuiGetItemID(void);                                            // get ID of last item (~~ often same ImGui::GetID(label) beforehand)
CIMGUI_API ImVec2  ImGuiGetItemRectMin(void);                                       // get upper-left bounding rectangle of the last item (screen space)
CIMGUI_API ImVec2  ImGuiGetItemRectMax(void);                                       // get lower-right bounding rectangle of the last item (screen space)
CIMGUI_API ImVec2  ImGuiGetItemRectSize(void);                                      // get size of last item

// Viewports
// - Currently represents the Platform Window created by the application which is hosting our Dear ImGui windows.
// - In 'docking' branch with multi-viewport enabled, we extend this concept to have multiple active viewports.
// - In the future we will extend this concept further to also represent Platform Monitor and support a "no main platform window" operation mode.
CIMGUI_API ImGuiViewport* ImGuiGetMainViewport(void);   // return primary/default viewport. This can never be NULL.

// Background/Foreground Draw Lists
CIMGUI_API ImDrawList* ImGuiGetBackgroundDrawList(void);   // this draw list will be the first rendered one. Useful to quickly draw shapes/text behind dear imgui contents.
CIMGUI_API ImDrawList* ImGuiGetForegroundDrawList(void);   // this draw list will be the last rendered one. Useful to quickly draw shapes/text over dear imgui contents.

// Miscellaneous Utilities
CIMGUI_API bool                  ImGuiIsRectVisibleBySize(ImVec2 size);                  // test if rectangle (of given size, starting from cursor position) is visible / not clipped.
CIMGUI_API bool                  ImGuiIsRectVisible(ImVec2 rect_min, ImVec2 rect_max);   // test if rectangle (in screen space) is visible / not clipped. to perform coarse clipping on user's side.
CIMGUI_API double                ImGuiGetTime(void);                                     // get global imgui time. incremented by io.DeltaTime every frame.
CIMGUI_API int                   ImGuiGetFrameCount(void);                               // get global imgui frame count. incremented by 1 every frame.
CIMGUI_API ImDrawListSharedData* ImGuiGetDrawListSharedData(void);                       // you may use this when creating your own ImDrawList instances.
CIMGUI_API const char*           ImGuiGetStyleColorName(ImGuiCol idx);                   // get a string corresponding to the enum value (for display, saving, etc.).
CIMGUI_API void                  ImGuiSetStateStorage(ImGuiStorage* storage);            // replace current window storage with our own (if you want to manipulate it yourself, typically clear subsection of it)
CIMGUI_API ImGuiStorage*         ImGuiGetStateStorage(void);

// Text Utilities
CIMGUI_API ImVec2 ImGuiCalcTextSize(const char* text);   // Implied text_end = NULL, hide_text_after_double_hash = false, wrap_width = -1.0f
CIMGUI_API ImVec2 ImGuiCalcTextSizeEx(const char* text, const char* text_end /* = NULL */, bool hide_text_after_double_hash /* = false */, float wrap_width /* = -1.0f */);

// Color Utilities
CIMGUI_API ImVec4 ImGuiColorConvertU32ToFloat4(ImU32 in);
CIMGUI_API ImU32  ImGuiColorConvertFloat4ToU32(ImVec4 in);
...
CIMGUI_API bool        ImGuiIsKeyDown(ImGuiKey key);                                             // is key being held.
CIMGUI_API bool        ImGuiIsKeyPressed(ImGuiKey key);                                          // Implied repeat = true
CIMGUI_API bool        ImGuiIsKeyPressedEx(ImGuiKey key, bool repeat /* = true */);              // was key pressed (went from !Down to Down)? if repeat=true, uses io.KeyRepeatDelay / KeyRepeatRate
CIMGUI_API bool        ImGuiIsKeyReleased(ImGuiKey key);                                         // was key released (went from Down to !Down)?
CIMGUI_API bool        ImGuiIsKeyChordPressed(ImGuiKeyChord key_chord);                          // was key chord (mods + key) pressed, e.g. you can pass 'ImGuiMod_Ctrl | ImGuiKey_S' as a key-chord. This doesn't do any routing or focus check, please consider using Shortcut() function instead.
CIMGUI_API int         ImGuiGetKeyPressedAmount(ImGuiKey key, float repeat_delay, float rate);   // uses provided repeat rate/delay. return a count, most often 0 or 1 but might be >1 if RepeatRate is small enough that DeltaTime > RepeatRate
CIMGUI_API const char* ImGuiGetKeyName(ImGuiKey key);                                            // [DEBUG] returns English name of the key. Those names are provided for debugging purpose and are not meant to be saved persistently nor compared.
CIMGUI_API void        ImGuiSetNextFrameWantCaptureKeyboard(bool want_capture_keyboard);         // Override io.WantCaptureKeyboard flag next frame (said flag is left for your application to handle, typically when true it instructs your app to ignore inputs). e.g. force capture keyboard when your widget is being hovered. This is equivalent to setting "io.WantCaptureKeyboard = want_capture_keyboard"; after the next NewFrame() call.
...
CIMGUI_API bool ImGuiShortcut(ImGuiKeyChord key_chord, ImGuiInputFlags flags /* = 0 */);
CIMGUI_API void ImGuiSetNextItemShortcut(ImGuiKeyChord key_chord, ImGuiInputFlags flags /* = 0 */);
...
CIMGUI_API void ImGuiSetItemKeyOwner(ImGuiKey key);   // Set key owner to last item ID if it is hovered or active. Equivalent to 'if (IsItemHovered() || IsItemActive()) { SetKeyOwner(key, GetItemID());'.

// Inputs Utilities: Mouse
// - To refer to a mouse button, you may use named enums in your code e.g. ImGuiMouseButton_Left, ImGuiMouseButton_Right.
// - You can also use regular integer: it is forever guaranteed that 0=Left, 1=Right, 2=Middle.
// - Dragging operations are only reported after mouse has moved a certain distance away from the initial clicking position (see 'lock_threshold' and 'io.MouseDraggingThreshold')
CIMGUI_API bool             ImGuiIsMouseDown(ImGuiMouseButton button);                                           // is mouse button held?
CIMGUI_API bool             ImGuiIsMouseClicked(ImGuiMouseButton button);                                        // Implied repeat = false
CIMGUI_API bool             ImGuiIsMouseClickedEx(ImGuiMouseButton button, bool repeat /* = false */);           // did mouse button clicked? (went from !Down to Down). Same as GetMouseClickedCount() == 1.
CIMGUI_API bool             ImGuiIsMouseReleased(ImGuiMouseButton button);                                       // did mouse button released? (went from Down to !Down)
CIMGUI_API bool             ImGuiIsMouseDoubleClicked(ImGuiMouseButton button);                                  // did mouse button double-clicked? Same as GetMouseClickedCount() == 2. (note that a double-click will also report IsMouseClicked() == true)
CIMGUI_API bool             ImGuiIsMouseReleasedWithDelay(ImGuiMouseButton button, float delay);                 // delayed mouse release (use very sparingly!). Generally used with 'delay >= io.MouseDoubleClickTime' + combined with a 'io.MouseClickedLastCount==1' test. This is a very rarely used UI idiom, but some apps use this: e.g. MS Explorer single click on an icon to rename.
CIMGUI_API int              ImGuiGetMouseClickedCount(ImGuiMouseButton button);                                  // return the number of successive mouse-clicks at the time where a click happen (otherwise 0).
CIMGUI_API bool             ImGuiIsMouseHoveringRect(ImVec2 r_min, ImVec2 r_max);                                // Implied clip = true
CIMGUI_API bool             ImGuiIsMouseHoveringRectEx(ImVec2 r_min, ImVec2 r_max, bool clip /* = true */);      // is mouse hovering given bounding rect (in screen space). clipped by current clipping settings, but disregarding of other consideration of focus/window ordering/popup-block.
CIMGUI_API bool             ImGuiIsMousePosValid(const ImVec2* mouse_pos /* = NULL */);                          // by convention we use (-FLT_MAX,-FLT_MAX) to denote that there is no mouse available
CIMGUI_API bool             ImGuiIsAnyMouseDown(void);                                                           // [WILL OBSOLETE] is any mouse button held? This was designed for backends, but prefer having backend maintain a mask of held mouse buttons, because upcoming input queue system will make this invalid.
CIMGUI_API ImVec2           ImGuiGetMousePos(void);                                                              // shortcut to ImGui::GetIO().MousePos provided by user, to be consistent with other calls
CIMGUI_API ImVec2           ImGuiGetMousePosOnOpeningCurrentPopup(void);                                         // retrieve mouse position at the time of opening popup we have BeginPopup() into (helper to avoid user backing that value themselves)
CIMGUI_API bool             ImGuiIsMouseDragging(ImGuiMouseButton button, float lock_threshold /* = -1.0f */);   // is mouse dragging? (uses io.MouseDraggingThreshold if lock_threshold < 0.0f)
CIMGUI_API ImVec2           ImGuiGetMouseDragDelta(ImGuiMouseButton button /* = 0 */, float lock_threshold /* = -1.0f */); // return the delta from the initial clicking position while the mouse button is pressed or was just released. This is locked and return 0.0f until the mouse moves past a distance threshold at least once (uses io.MouseDraggingThreshold if lock_threshold < 0.0f)
CIMGUI_API void             ImGuiResetMouseDragDelta(void);                                                      // Implied button = 0
CIMGUI_API void             ImGuiResetMouseDragDeltaEx(ImGuiMouseButton button /* = 0 */);                       //
CIMGUI_API ImGuiMouseCursor ImGuiGetMouseCursor(void);                                                           // get desired mouse cursor shape. Important: reset in ImGui::NewFrame(), this is updated during the frame. valid before Render(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
CIMGUI_API void             ImGuiSetMouseCursor(ImGuiMouseCursor cursor_type);                                   // set desired mouse cursor shape
CIMGUI_API void             ImGuiSetNextFrameWantCaptureMouse(bool want_capture_mouse);                          // Override io.WantCaptureMouse flag next frame (said flag is left for your application to handle, typical when true it instructs your app to ignore inputs). This is equivalent to setting "io.WantCaptureMouse = want_capture_mouse;" after the next NewFrame() call.

// Clipboard Utilities
// - Also see the LogToClipboard() function to capture GUI into clipboard, or easily output text data to the clipboard.
CIMGUI_API const char* ImGuiGetClipboardText(void);
...
CIMGUI_API void        ImGuiLoadIniSettingsFromDisk(const char* ini_filename);                            // call after CreateContext() and before the first call to NewFrame(). NewFrame() automatically calls LoadIniSettingsFromDisk(io.IniFilename).
CIMGUI_API void        ImGuiLoadIniSettingsFromMemory(const char* ini_data, size_t ini_size /* = 0 */);   // call after CreateContext() and before the first call to NewFrame() to provide .ini data from your own data source.
CIMGUI_API void        ImGuiSaveIniSettingsToDisk(const char* ini_filename);                              // this is automatically called (if io.IniFilename is not empty) a few seconds after any modification that should be reflected in the .ini file (and also by DestroyContext).
CIMGUI_API const char* ImGuiSaveIniSettingsToMemory(size_t* out_ini_size /* = NULL */);                   // return a zero-terminated string with the .ini data which you can save by your own mean. call when io.WantSaveIniSettings is set, then save data by your own mean and clear io.WantSaveIniSettings.

// Debug Utilities
// - Your main debugging friend is the ShowMetricsWindow() function, which is also accessible from Demo->Tools->Metrics Debugger
CIMGUI_API void  ImGuiDebugTextEncoding(const char* text);
CIMGUI_API void  ImGuiDebugFlashStyleColor(ImGuiCol idx);
CIMGUI_API void  ImGuiDebugStartItemPicker(void);
CIMGUI_API bool  ImGuiDebugCheckVersionAndDataLayout(const char* version_str, size_t sz_io, size_t sz_style, size_t sz_vec2, size_t sz_vec4, size_t sz_drawvert, size_t sz_drawidx); // This is called by IMGUI_CHECKVERSION() macro.
#ifndef IMGUI_DISABLE_DEBUG_TOOLS
CIMGUI_API void ImGuiDebugLog(const char* fmt, ...) IM_FMTARGS(1);             // Call via IMGUI_DEBUG_LOG() for maximum stripping in caller code!
CIMGUI_API void ImGuiDebugLogUnformatted(const char* text);                    // Call via IMGUI_DEBUG_LOG() for maximum stripping in caller code!
CIMGUI_API void ImGuiDebugLogV(const char* fmt, va_list args) IM_FMTLIST(1);
#endif // #ifndef IMGUI_DISABLE_DEBUG_TOOLS
// Memory Allocators
// - Those functions are not reliant on the current context.
// - DLL users: heaps and globals are not shared across DLL boundaries! You will need to call SetCurrentContext() + SetAllocatorFunctions()
//   for each static/DLL boundary you are calling from. Read "Context and Memory Allocators" section of imgui.cpp for more details.
CIMGUI_API void  ImGuiSetAllocatorFunctions(ImGuiMemAllocFunc alloc_func, ImGuiMemFreeFunc free_func, void* user_data /* = NULL */);
CIMGUI_API void  ImGuiGetAllocatorFunctions(ImGuiMemAllocFunc* p_alloc_func, ImGuiMemFreeFunc* p_free_func, void** p_user_data);
CIMGUI_API void* ImGuiMemAlloc(size_t size);
...
CIMGUI_API void   ImGuiSetWindowFontScale(float scale);                                                       // Set font scale factor for current window. Prefer using PushFont(NULL, style.FontSizeBase * factor) or use style.FontScaleMain to scale all windows.
// OBSOLETED in 1.91.9 (from February 2025)
CIMGUI_API void   ImGuiImageImVec4(ImTextureRef tex_ref, ImVec2 image_size, ImVec2 uv0, ImVec2 uv1, ImVec4 tint_col, ImVec4 border_col); // <-- 'border_col' was removed in favor of ImGuiCol_ImageBorder. If you use 'tint_col', use ImageWithBg() instead.
// OBSOLETED in 1.91.0 (from July 2024)
CIMGUI_API void   ImGuiPushButtonRepeat(bool repeat);
CIMGUI_API void   ImGuiPopButtonRepeat(void);
CIMGUI_API void   ImGuiPushTabStop(bool tab_stop);
CIMGUI_API void   ImGuiPopTabStop(void);
CIMGUI_API ImVec2 ImGuiGetContentRegionMax(void);                                                             // Content boundaries max (e.g. window boundaries including scrolling, or current column boundaries). You should never need this. Always use GetCursorScreenPos() and GetContentRegionAvail()!
CIMGUI_API ImVec2 ImGuiGetWindowContentRegionMin(void);                                                       // Content boundaries min for the window (roughly (0,0)-Scroll), in window-local coordinates. You should never need this. Always use GetCursorScreenPos() and GetContentRegionAvail()!
CIMGUI_API ImVec2 ImGuiGetWindowContentRegionMax(void);                                                       // Content boundaries max for the window (roughly (0,0)+Size-Scroll), in window-local coordinates. You should never need this. Always use GetCursorScreenPos() and GetContentRegionAvail()!
// OBSOLETED in 1.90.0 (from September 2023)
CIMGUI_API bool   ImGuiBeginChildFrame(ImGuiID id, ImVec2 size);                                              // Implied window_flags = 0
CIMGUI_API bool   ImGuiBeginChildFrameEx(ImGuiID id, ImVec2 size, ImGuiWindowFlags window_flags /* = 0 */);
CIMGUI_API void   ImGuiEndChildFrame(void);
...
CIMGUI_API void   ImGuiShowStackToolWindow(bool* p_open /* = NULL */);
CIMGUI_API bool   ImGuiComboObsolete(const char* label, int* current_item, bool (*old_callback)(void* user_data, int idx, const char** out_text), void* user_data, int items_count); // Implied popup_max_height_in_items = -1
CIMGUI_API bool   ImGuiComboObsoleteEx(const char* label, int* current_item, bool (*old_callback)(void* user_data, int idx, const char** out_text), void* user_data, int items_count, int popup_max_height_in_items /* = -1 */);
CIMGUI_API bool   ImGuiListBoxObsolete(const char* label, int* current_item, bool (*old_callback)(void* user_data, int idx, const char** out_text), void* user_data, int items_count); // Implied height_in_items = -1
CIMGUI_API bool   ImGuiListBoxObsoleteEx(const char* label, int* current_item, bool (*old_callback)(void* user_data, int idx, const char** out_text), void* user_data, int items_count, int height_in_items /* = -1 */);
